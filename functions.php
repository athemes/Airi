...
if ( ! function_exists( 'airi_font_scripts' ) ) {
	function airi_font_scripts() {
		$headings_font = esc_html(get_theme_mod('typography_headings_font'));
		$body_font = esc_html(get_theme_mod('typography_body_font'));
...
if ( ! function_exists( 'airi_font_styles' ) ) {
	function airi_font_styles() {
		...
		if ( $headings_font ) {
			...
			$custom .= "h1, h2, h3, h4, h5, h6 { font-family: $font; }"."\n";
		}

		if ( $body_font ) {
			$font_pieces = $font_pieces = airi_process_font($body_font);
			$font = htmlspecialchars_decode($font_pieces[0]);
			$custom .= "body, button, input, select, textarea { font-family: $font; }"."\n";
		}

		// Output all styles
		echo '<style id="airi-font-css">' . $custom . '</style>';
